...
We have used hashmaps to ensure average $O(1)$ bucket update time.
In order to compare the net allocation number between successive
analysis phases, one has to maintain hashmaps from the previous phase
and operate on fresh ones.

Another hashmap is necessary to keep track sizes of
...
Indeed, for each bucket we need to update the hashmap
...
\comment{Simon suggested to refer vaguely to 'Google infrastructure'.
Do we want to keep the reference to \cite{gwp}?}
